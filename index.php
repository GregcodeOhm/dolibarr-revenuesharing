...
$sql_collab = "SELECT COUNT(*) as nb_collaborators FROM ".MAIN_DB_PREFIX."revenuesharing_collaborator WHERE active = 1";
$resql_collab = $db->query($sql_collab);
$nb_collaborators = 0;
if ($resql_collab) {
    $obj = $db->fetch_object($resql_collab);
    $nb_collaborators = $obj->nb_collaborators;
    $db->free($resql_collab);
}

// 2. Statistiques des contrats pour l'année
...
$sql_contracts .= " WHERE YEAR(date_creation) = ".$year;
if ($filter_collaborator > 0) {
    $sql_contracts .= " AND fk_collaborator = ".((int) $filter_collaborator);
}
...
if ($resql_contracts) {
    $obj = $db->fetch_object($resql_contracts);
    $stats = array(
        'nb_contracts' => $obj->nb_contracts ? $obj->nb_contracts : 0,
        'nb_draft' => $obj->nb_draft ? $obj->nb_draft : 0,
        'nb_valid' => $obj->nb_valid ? $obj->nb_valid : 0,
        'total_ht' => $obj->total_ht ? $obj->total_ht : 0,
        'total_collaborator' => $obj->total_collaborator ? $obj->total_collaborator : 0,
        'total_studio' => ($obj->total_ht ? $obj->total_ht : 0) - ($obj->total_collaborator ? $obj->total_collaborator : 0)
    );
