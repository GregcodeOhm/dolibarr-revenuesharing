...
```javascript
masseSalariale = totalCachets × 1.45
```
- Inclut le salaire brut + charges employeur (~45%)
...
function updateFinancialFields(totalDays, totalHeures) {
    ...
    if (cachetUnitaire > 0 && totalDays > 0) {
        ...
        const masseSalariale = totalCachets * 1.45;
...
```
Cachet brut:        150,00 €
Charges employeur:   67,50 € (45%)
Masse salariale:    217,50 €
```
...
3. **Calculs automatiques** :
   ...
   - Masse salariale : 1 087,50 €
   - Solde utilisé : 1 087,50 €
...
```javascript
const tauxCharges = 1.50; // Pour 50% au lieu de 45%
```
